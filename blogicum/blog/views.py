...
class IndexListView(ListView):
    ...
    def suitable_posts(self):
        return Post.objects.select_related(
             ...
             ).order_by('pub_date__lte').annotate(comment_count=Count('comments'))


class Post_addCreateView(PostMixin, LoginRequiredMixin, CreateView):
    form_class = PostForm
...
class Post_detailDetailView(DetailView):
    model = Post
...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.post_obj = get_object_or_404(Post, pk=kwargs['post_id'], published=True)
        return super().dispatch(request, *args, **kwargs)

    def form_valid(self, form, *args, **kwargs):
        form.instance.author = self.request.user
        form.instance.post = get_object_or_404(Post, pk=kwargs['post_id'], published=True)
        return super().form_valid(form)
